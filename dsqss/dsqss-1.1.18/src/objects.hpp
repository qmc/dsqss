...
public:
  ...
  void setONINTERACTION( Interaction* on_interaction ){ONINTERACTION=on_interaction;};
...
public:
  ...
  bool getUORD(){return ( _vcur == &( (*_scur).top() ));};//up -> false 0, down -> true 1
  void getV(){printf("top,bottom = %d, %d ",(*_scur).top().id(),(*_scur).bottom().id());};
...
private:
  ...
  Interaction** ConnectedInteraction; //Interaction* ConnectedInteraction [ NCI ];
public:
  
  void setNCI(int ni){ NCI = ni;  ConnectedInteraction = new Interaction* [ni]; };
  ...
  void init(SiteProperty& SP, int mt) { _SP = &SP; MTYPE = mt; };
  /*edit by masaki->*/
  void init(SiteProperty& SP, int mt, double cq, double sq ) {
    _SP = &SP;
    MTYPE = mt;
    COSQ = cq;
    SINQ = sq;
  ...
  double getCOSQ() { return COSQ; };
  double getSINQ() { return SINQ; };
...
inline double BareSegment::topTime() {
  return top().time(); 
}
...
inline double BareSegment::length() { 
...
//======================================================================
inline void BareVertex::init_TERM(Segment& S) { // initializer for a terminal vertex//katou
  init();
  _s.init(1,2);
  _s.set_all(0);
  _s[0] = &S;
...
inline VertexInitialConfiguration& BareVertex::getInitialConfiguration( int inc , int xinc ) {
...
inline bool operator<(const RegisteredVertexInfo& obj0, const RegisteredVertexInfo& obj){
