...
class dgematrix{
 ...
 public:
  ...
  dgematrix(int _n, int _m){
     resize(_m, _n);
    for(int i=0; i<m*n; i++){ index[i] = 0.0; }
  };
  ...
  inline dgematrix& zero(){
    for(long i=0; i<m*n; i++){ index[i] =0.0; }
    return *this;
  }

  inline dgematrix& identity(){
    for(int i=0; i<m*n; i++){ index[i] =0.0; }
    for(int i=0; i<m; i++){ operator()(i,i) =1.0; }
    return *this;
  }

  inline dgematrix& operator*=(const  dgematrix&);
  inline double & operator()(const int &i, const int &j) { return index[i+j*m]; };
  inline double operator()(const int &i, const int &j)const { return index[i+j*m]; };
