...
bool GraphSpace::Next_event(My_rdm *MR){
  ...
  if( type == 0 )    return update_selection(MR, dir);
  else if( type < 3 && type > 0 )    return update_selection(MR, dir, type);
#ifdef SERIAL_S
  /*  else if( type == -5 ){ //edge on temporally axis

    int xl = worm->i%V;
    bool oh = worm->type-4;
    
    ( oh==dir  )? (world[xl].p)-- : (world[xl].p)++; 

    worldB[xl].p = world[xl].p;
    Wlen += LengthofWalk( vertex[0]->t, worm->t, dir);

     //ワームをほどく
    worm->next[0]->next[1] = worm->next[1];
    worm->next[1]->next[0] = worm->next[0];
    vertex[0] = vertex[0]->next[dir];

     //ワームをつなぎかえる
    worm->next[!dir] = vertex[0];
    worm->next[dir] = vertex[0]->next[dir];
    vertex[0]->next[dir]->next[!dir] = worm;
    vertex[0]->next[dir] = worm;
...
void GraphSpace::transition(My_rdm *MR){
  ...
  case 2: //Kink
    ...
    if( !dir ) nj = worm->p; //b=2;
    else nj = vertex[0]->p; //b=3;
