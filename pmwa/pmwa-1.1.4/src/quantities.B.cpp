...
Quantities::Quantities( Size *m_N, MC_p *m_MC, System *m_sp, Lattice *m_LT, Parallel *m_PR, char outfile[128] ){
  ...
  for(int i=0;i<V;i++){
    ...
    double phasep= (x+y+z)*PI;
    
    if((x+y+z)%2==0)  COSnp[i]=1.0;
    else COSnp[i]=-1.0;
    //COSnp[i]=cos(phasep);
    //SINnp[i]=sin(phasep);
...
void Quantities::BINsum(int bin){
  ...
  if(PR->nst==0) BINsum(MCmean_S, BINmean_S, Nq*2, bin);
  if(PR->nst==0)  BINsum(MCmean_L, BINmean_L, Lsize, bin);
}

void Quantities::BINaverage(){

  if(PR->nst==0) Average(BINmean_S, MC->Nbin, Nq*2, MCmean_S, 1);
  if(PR->nst==0)  Average(BINmean_L, MC->Nbin, Lsize*2, MCmean_L, 2 );
...
void Quantities::Density(GraphSpace::Vertex *world, GraphSpace::Vertex *worldB ){
  ...
  for( int i=0; i<V; i++ ){
    ...
    //    ctot += world[i].p*COSnp[i];
    //    stot += world[i].p*SINnp[i];
    stot += a0*LT->lx[i]; //COSnp[i];
    xtot += values_L[f_ld(i)]*LT->lx[i]; //COSnp[i];
  }
  ...
  //  values_S[sku] = atot; //Structure factor
  
   ///////////////// Sk //////////////////////
    
  //  values_S[sku] = atot*atot/(double)N->V; //Structure factor
  //  values_S[sks] = (ctot*ctot + stot*stot)/(double)N->V; //Structure factor
  //values_S[sks] = stot*stot/(double)N->V; //Structure factor
  //  Ck[Nk_set] = atot; //Structure factor
  //  Ck[Nk_set+1] = stot; //Structure factor
...
void Quantities::NumberOfWorms( vector<GraphSpace::Vertex> &ev, int mcs  ){
...
void Quantities::NoiseCorrelation( ){
  ...
  MCmean_S[d00*2] = MCmean_L[f_noise(0,0)];
  MCmean_S[d00*2+1] = MCmean_S[d00*2]*MCmean_S[d00*2];
