...
void WriteXML(int D, int L[], int Ntau, int CutoffOfNtau, int KTYPE) {
  ...
  if(KTYPE==0) KMAX=L[0]/2 + 1;
  else if(KTYPE==1) KMAX=4;
  else if(KTYPE==2) KMAX=3;
  else {
  ...
  for (int q=0; q<KMAX; q++) {
     ...
     if(KTYPE==0) ksite[q]=2*q;
     else if(KTYPE==1) ksite[q]=q;
    fout<<"<Kind> "<<" "<<q<<" "<<ksite[q]<<" </Kind>"<<endl;
  ...
  for(int i=0; i<N; i++){
	...
	double qx;
	double qy;
	if(q==0){qx=1.0; qy=0.0;}
	else if(q==1){qx=1.0; qy=1.0;}
	else if(q==2){qx=0.0; qy=1.0;}
	else if(q==3){qx=0.5; qy=0.5;}
	...
	double qx;
	double qy;
	if(q==0)     {qx=7.0/8.0; qy=1.0/8.0;}
	else if(q==1){qx=3.0/4.0; qy=1.0/4.0;}
	else if(q==2){qx=5.0/8.0; qy=3.0/8.0;}
...
int main(int argc,char** argv){
  ...
  int Ntau = atoi(argv[iarg]); iarg++;

  int Ntcut = atoi(argv[iarg]); iarg++;
