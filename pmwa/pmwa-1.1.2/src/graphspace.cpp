...
GraphSpace::GraphSpace(Size *N,int m_nmax,Lattice *m_LT,Probability *m_P, Parallel *m_PR,  long long  m_IMAX,  long long  m_WMAX, char m_Eventfile_old[128],int nc, My_rdm *MR, bool cb, char m_outfile[128] ){
...
void GraphSpace::initialev( char Eventfile_old[128],int nc, My_rdm *MR, bool cb, double oldB ){
  ...
  if(fin){
    ...
    Ncyc = 30; 
    if(cb) for(int i=0; i<V; i++)    world[i].p = worldB[i].p =( ( i%PR->x + (int)(i/PR->x) )%2==0 )? 1:0;
    else for(int i=0; i<V; i++)    world[i].p = worldB[i].p = (nmax+1)*MR->rdm();
  }
