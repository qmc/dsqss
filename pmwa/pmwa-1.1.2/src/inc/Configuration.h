...
class GraphSpace{
  ...
  double rtb_i,rt_i, rv2_i, rtb_tot, rt_tot, rt_frame, rv2_tot, rtot;
  double Pv2, Pv1,PBv2, PBv1;
  double Wlen;
  long Ncyc;
  int Nv, Nk, Nu;
  double OPcount;
  ...
  GraphSpace(Size *N, int m_nmax,Lattice *m_LT,Probability *m_P,Parallel *PR,  long long  m_IMAX,  long long  m_WMAX, char m_Eventfile[128],int nc, My_rdm *MR, bool cb, char m_outfile[128] );
  ~GraphSpace();
  ...
  inline int f( int d, int i, int j){ return i+j*LT->Fx[d]; }//world->(x座標,y座標)、Box->(1次元的座標,バーテックス)
  ...
  //vertex
  void initialev( char m_Eventfile[128],int nc, My_rdm *MR, bool cb, double oldB);
...
class Configuration : public GraphSpace {
  ...
  Configuration( MC_p *m_MC, Size *N, int m_nmax,Lattice *m_LT, Probability *m_P,Parallel *PR,  long long  m_IMAX,  long long  m_WMAX, char m_Eventfile[128], My_rdm *MR, bool cb, char m_outfile[128] );
  ~Configuration();
