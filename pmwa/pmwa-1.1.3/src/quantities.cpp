...
Quantities::Quantities( Size *m_N, MC_p *m_MC, System *m_sp, Lattice *m_LT, Parallel *m_PR, char outfile[128] ){
  ...
  sprintf(Qname[wnd2],"wnd2 "); //Winding Number
  sprintf(Qname[wndx],"wndx "); //Winding Number
  sprintf(Qname[wndy],"wndy "); //Winding Number
  sprintf(Qname[wndz],"wndz "); //Winding Number
#endif
  ...
  */
  ...
  newcall(m_val,max(max(Lsize,(int)Nq),max(Lmax[1]*3,V*2)));
...
void Quantities::Density(GraphSpace::Vertex *world, GraphSpace::Vertex *worldB ){
  ...
  double atot=0.0, btot=0.0, stot=0.0;
  ...
  for( int i=0; i<V; i++ ){
    ...
    btot += n0/N->B;
    atot += (world[i].p-0.5);
    //    stot += world[i].p*COSnp[i];

  }

  if(PR->nt==0) values_S[amzu] = atot/(double)N->V; //Density 
  values_S[bmzu] = btot/(double)N->V; //Density 
...
void Quantities::Compressibility(){
  ...
  MCmean_S[comp*2] = N->V * (MCmean_S[bmzu*2+1] / (MCmean_S[bmzu*2]*MCmean_S[bmzu*2]) - 1.0);
...
void Quantities::Susceptibility(){
  ...
  MCmean_S[xmx*2]   = (MCmean_S[nwor*2+1] - MCmean_S[nwor*2]*MCmean_S[nwor*2] - MCmean_S[nwor*2])/(N->V*N->B*sp->Htr*sp->Htr);
...
//  TreeSum( Nq, values_S, MC->Nsample , PR->nst, PR->nst0, PR->NtNs );
  ...
  if ( PR->nst==0 ){//Sum over t and s
    ...
    for(int tag=1; tag< PR->NtNs; tag++ ){
      ...
      for(int i=0; i<Nq; i++){	values_S[i] += m_val[i]; }
    }
