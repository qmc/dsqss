...
Quantities::Quantities( Size *m_N, MC_p *m_MC, System *m_sp, Lattice *m_LT, Parallel *m_PR, char outfile[128] ){
  ...
  sprintf(Qname[wind],"Winding "); //Winding Number
  sprintf(Qname[magz],"Density "); //Density or z-Magnetization
  sprintf(Qname[ene] ,"Energy  "); //Energy
  sprintf(Qname[nver],"Nvertex "); //Number of vertices
  sprintf(Qname[spe] ,"SpeHeat "); //Specific heat
  sprintf(Qname[nwor],"Nworm   "); //Number of worms
  sprintf(Qname[susx],"Susce_x "); //Susceptibility
  sprintf(Qname[nkin],"Nkink   ");  //Number of kinks
  sprintf(Qname[comp],"Compress");  //Compressibility
  sprintf(Qname[magx],"BEC_x   ");  //BEC order parameter or x-Magnet
  sprintf(Qname[magp],"BEC_p   ");  //BEC order parameter or x-Magnet
  sprintf(Qname[magm],"BEC_m   ");  //BEC order parameter or x-Magnet

  sprintf(Qname[Nq],"mz");  //Local density or local mz 
  sprintf(Qname[Nq+1],"cr2"); //2-points correlation (mx-mx)
  sprintf(Qname[Nq+2],"sk");  //structure factor
...
void Quantities::Energy( int countv, int mcs ){
   ...
   values_S[ene] = (sp->Ev2 + sp->Eu + sp->Et - (double)countv/PR->B)/(double)(N->V*PR->Ntdiv);
